...
const TestScreen = () => {
  ...
  const fetchDriverId = async () => {
    try {
      if (deviceId) {
        const response = await axios.post('http://192.168.8.131:4000/api/driver/device', {
  ...
  useEffect(() => {
    ...
    const socket = io('http://192.168.8.131:4000'); // Replace with your server address
  ...
  const updateDriverAvailability = async (newIsEnabled) => {
    try {
      if (driverId) {
        await axios.post('http://192.168.8.131:4000/api/driver/updateAvailability', {
